...
my @inputvars = qw(CASE_KEY residence_nh residence rent_cost selfemployed_netprofit_total); #Add as needed to determing successful execution of modules or to debug them.

#Removing these output variables to conserve time and space in processing. Just throw them back in if needed:
# rent_cost_m fpl
 
my @outputvars = qw(parent1_employedhours_w parent2_employedhours_w parent3_employedhours_w parent4_employedhours_w parent1_earnings_m parent2_earnings_m parent3_earnings_m parent4_earnings_m ssi_recd hlth_cov_parent1 hlth_cov_parent2 hlth_cov_parent3 hlth_cov_parent4 hlth_cov_child_all health_expenses child_care_expenses ccdf_step child_care_recd rent_paid housing_recd fsp_recd cadc_recd eitc_recd payroll_tax tax_before_credits federal_tax_credits trans_expenses food_expenses child_foodcost_red_total other_expenses lifeline_recd salestax wic_recd liheap_recd eap_recd tanf_recd child_support_recd
income expenses net_resources); #Add as needed to determing successful execution of modules or to debug them.
...
while (my $line = <TEST1>) {
	...
	if ($. == 1) {
		...
		#Now we reassign the names of the input variables from New HEIGHTS that don't have the same exact names as the variables in the FRS codes. Follow the same syntax as below but like follows:		
		$self{'in'}->{'mother_child1_PERSON_KEY'} = $self{'in'}->{'child1_mother_PERSON_KEY'};
		$self{'in'}->{'mother_child2_PERSON_KEY'} = $self{'in'}->{'child2_mother_PERSON_KEY'};
		$self{'in'}->{'mother_child3_PERSON_KEY'} = $self{'in'}->{'child3_mother_PERSON_KEY'};
		$self{'in'}->{'mother_child4_PERSON_KEY'} = $self{'in'}->{'child4_mother_PERSON_KEY'};
		$self{'in'}->{'mother_child5_PERSON_KEY'} = $self{'in'}->{'child5_mother_PERSON_KEY'};
		$self{'in'}->{'residence_nh'} = $self{'in'}->{'city'};
		$self{'in'}->{'debt_payment_m'} = $self{'in'}->{'debt_amount_m'};
		$self{'in'}->{'disability_medical_expenses_mnth'} = $self{'in'}->{'disability_medical_expenses'};
		$self{'in'}->{'disability_personal_expenses_m'} = $self{'in'}->{'disability_personal_expenses'};
		$self{'in'}->{'disability_work_expenses_m'} = $self{'in'}->{'disability_work_expenses'};
		$self{'in'}->{'FAP_Cash_initial'} = $self{'in'}->{'FAP_cash_initial'};
		$self{'in'}->{'food_override'} = $self{'in'}->{'food_cost_override'};
		$self{'in'}->{'food_override_amt'} = $self{'in'}->{'food_cost_override_amt'};
		$self{'in'}->{'FWOC_Cash_initial'} = $self{'in'}->{'FWOC_cash_initial'};
		$self{'in'}->{'housing_override'} = $self{'in'}->{'housing_cost_override'};
		$self{'in'}->{'housing_override_amt'} = $self{'in'}->{'housing_cost_override_amt'};
#		$self{'in'}->{'garnishments'} = $self{'in'}->{'id'};
		$self{'in'}->{'NHEP_Cash_initial'} = $self{'in'}->{'NHEP_cash_initial'};
		$self{'in'}->{'IDP_Cash_initial'} = $self{'in'}->{'IDP_cash_initial'};
		$self{'in'}->{'other_override'} = $self{'in'}->{'other_cost_override'};
		$self{'in'}->{'other_override_amt'} = $self{'in'}->{'other_cost_override_amt'};
		$self{'in'}->{'disability_parent1'} = $self{'in'}->{'parent1_disab_flag'};
		$self{'in'}->{'parent1_employedhours_initial_m'} = $self{'in'}->{'parent1_employedhours_initial'};
		$self{'in'}->{'parent1_ssi_recd_m_initial'} = $self{'in'}->{'parent1_ssi_recd_initial'};
		$self{'in'}->{'disability_parent2'} = $self{'in'}->{'parent2_disab_flag'};
		$self{'in'}->{'parent2_employedhours_initial_m'} = $self{'in'}->{'parent2_employedhours_initial'};
		$self{'in'}->{'parent2_ssi_recd_m_initial'} = $self{'in'}->{'parent2_ssi_recd_initial'};
		$self{'in'}->{'disability_parent3'} = $self{'in'}->{'parent3_disab_flag'};
		$self{'in'}->{'parent3_employedhours_initial_m'} = $self{'in'}->{'parent3_employedhours_initial'};
		$self{'in'}->{'parent3_ssi_recd_m_initial'} = $self{'in'}->{'parent3_ssi_recd_initial'};
		$self{'in'}->{'disability_parent4'} = $self{'in'}->{'parent4_disab_flag'};
		$self{'in'}->{'parent4_employedhours_initial_m'} = $self{'in'}->{'parent4_employedhours_initial'};
		$self{'in'}->{'parent4_ssi_recd_m_initial'} = $self{'in'}->{'parent4_ssi_recd_initial'};
		$self{'in'}->{'userplan_type'} = $self{'in'}->{'userplantype'};
		#Setting vehicle values to 0, since they do not appear in the New HEIGHTS data nor ESI's fixed variables included in the csv file:
		foreach my $name (qw(vehicle1_owed vehicle1_value vehicle2_owed vehicle2_value)) { 
			$self{'in'}->{$name} = 0; 
		}
		
		#TEMPORARY CORRECTIONS (comment out or delete as ESI corrects them:
		for(my $i=1; $i<=5; $i++) {
			if ($self{'in'}->{'child'.$i.'_age'} eq 'NA') {
				$self{'in'}->{'child'.$i.'_age'} = -1;
			}
			#if ($self{'in'}->{'child'.$i.'_age'} >= 0) {
			#	$self{'in'}->{'mother_child'.$i.'_PERSON_KEY'} = $self{'in'}->{'parent1_PERSON_KEY'};
			#}
		}
		...
		foreach my $name (qw(ui_recd_initial ownhome valueofownhome)) { #REQUESTING THAT THIS BE ADDED TO NEW HEIGHTS DATA SET. BUT FOR NOW, NEEDS TO BE SET AT ZERO SO THAT CODES THAT USE IT CAN RUN APPROPRIATELY. MAY END UP NOT USING THIS AT ALL BUT MAY BE USEFUL.  
			$self{'in'}->{$name} = 0;
		}
		#Some codes refer to parent disability without checking first whether the parent is in the house.
		for(my $i=1; $i<=4; $i++) {
