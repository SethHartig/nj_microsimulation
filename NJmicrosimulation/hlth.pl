...
sub hlth
{
	...
	if ($in->{'hlth'} == 0 && $in->{'premium_tax_credit'} == 0) { 
		...
		if ($in->{'premium_tax_credit'} == 1 && ($out->{'percent_of_poverty'} <= $sub_maximum || $in->{'covid_ptc_expansion'} == 1) && ($out->{'percent_of_poverty'} >= $sub_minimum || $parent_greencard_premiums + $family_greencard_premiums > 0) && ($in->{'undocumented_adult_count'} == 0 || $in->{'daca_adult_count'} == 0 || $in->{'itin'} == 1)) {
			#Note about adding in last condition about undocumented immigrants: The income of undocumented adults in a home is calculated toward PTC and Medicaid eligibility of other household members (see https://www.kff.org/faqs/faqs-health-insurance-marketplace-and-the-aca/can-family-members-in-families-with-mixed-immigration-status-where-some-family-members-are-citizens-or-lawfully-present-and-others-are-undocumented-enroll-in-medicaid-or-chip-or-receive-help-buying/), but in order to claim the PTC, a family needs to file taxes. This means that in families in which wage earners do not have an SSN or ITIN, no one can claim PTCs without committing fraud. This code is NOT making undocuamnted parents with ITINs eligible for the PTC, because the parent_cost_individual calculation is not inclusive of them; their premiums come from the addition of the nonmarketplace individual calculations. So this is only calculating premium subsidies for adults who are eligible for the PTC. 
