#=============================================================================#
#  Federal Health Insurance  Module – 2021 (revised from 2020 )
#=============================================================================#
#
# Inputs and outputs referenced in this module:
#
#   FROM BASE (either user inputs or generated in the frs.pm code)
#       earnings_m
#       interest_m
#		family_size
#		undocumented_adult_count
#		parent1_immigration_status
#		covid_ptc_ui_expansion
#		spousal_sup_ncp
#
#   FROM INTEREST 
#       interest_m
...
sub fed_hlth_insurance
{
    ...
    our $max_income_pct_employer = 0.0961;    #  Maximum percentage of income dedicated for self-coverage under employer-based insurance. This is the required contribution percentage for the 2022 tax year, as indicated at https://www.irs.gov/pub/irs-irbs/irb21-35.pdf. 
    ...
    our $private_max = 0;                     #   Maximum payment toward coverage of Second Lowest Priced Silver Plan
    our $percent_of_poverty = 0;              #   Income as representative of  percent of applicable poverty level

    # OTHER VARIABLES USED  

    our $subsidy_pov_level_ssi = 0;
    our $percent_of_poverty_min = 0;          # See table
    our $percent_of_poverty_max = 0;          # See table
    our $premium_cap_pct = 0;                 # See table
	our $percent_of_poverty_ssi = 0;		 #new for 2017
	...
	# EVALUATE ELIGIBILITY for premium tax credits. Generally, if your filing status is married filing separately you cannot obtain the credit, and undocumented persons cannot take advantage of the premium tax credit. However, "Certain aliens with household income below 100% of the federal poverty line are not eligible for Medicaid because of their immigration status. You may qualify for the PTC if your household income is less than 100% of the federal poverty line if you..." (1) you or an individual in your tax family enrolled in a Marketplace plan, (2) the enrollee is lawfully present in U.S. and not eligible for Medicaid b/c of immigration status, and you otherwise qualify as an applicable taxpayer except for the FPL requirement. See https://www.irs.gov/instructions/i8962#idm140230440064224 for more info.DACA recipients are not eligible for the PCT see wording that calls out daca recipients as specifically not being considered "lawfully present" for the PTC. https://www.law.cornell.edu/cfr/text/45/152.2

																															 
	# 1.  Calculate Modified Adjusted Gross Income (MAGI) 
	$hlth_gross_income_m = $out->{'earnings_mnth'} + $out->{'interest_m'} + $out->{'fli_plus_tdi_recd'}/12 + $out->{'ui_recd'}/12 + $in->{'spousal_sup_ncp'}/12; #child support is not included in MAGI - it is nontaxable. 
		
	#remove the income of the undocumented or daca recipient from the household income.
	if ($in->{'undocumented_adult_count'} > 0 || $in->{'daca_adult_count'} > 0) {
		for (my $i = 1; $i <= $in->{'family_structure'}; $i++)	{
			if (($in->{'parent'.$i.'_immigration_status'} eq 'undocumented_or_other') ||$in->{'parent'.$i.'_immigration_status'} eq	'daca') {
				$hlth_gross_income_m  -= $out->{'parent'.$i.'_earnings_m'} + $out->{'interest_m'} / $in->{'family_structure'};
			}
		}
		if (($in->{'parent1_immigration_status'} eq 'undocumented_or_other') || $in->{'parent1_immigration_status'} eq	'daca') {
			$hlth_gross_income_m  -= $in->{'spousal_sup_ncp'}/12;	#spousal support is assigned to the first parent
		}	
	}	

	#Note: Undocumented and DACA recipients are not eligible for the PTC. In a family with all parents ineligible for premium tax credits but children eligible for them, and Medicaid, no adjustments are needed, since the hlth_gross_income is 0 and they will be income eligible for Medicaid, and will not  be income-eligible for premium tax credits. 
    ...
    $percent_of_poverty = $hlth_gross_income_m / $subsidy_pov_level_m;
 
	if ($in->{'ssi_recd'} > 0) {
		$hlth_gross_income_m_ssi = $hlth_gross_income_m;
		
		if ($in->{'parent1_ssi'} == 1) {
		$hlth_gross_income_m_ssi = &pos_sub($hlth_gross_income_m_ssi, $out->{'parent1_earnings_m'});
		}
		
		if ($in->{'parent2_ssi'} == 1) {
			$hlth_gross_income_m_ssi = &pos_sub($hlth_gross_income_m_ssi, $out->{'parent2_earnings_m'});
		}
	...
	if ($out->{'ui_recd'} > 0 && $in->{'covid_ptc_ui_expansion'} == 1) {
